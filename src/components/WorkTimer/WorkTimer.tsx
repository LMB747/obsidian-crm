...
import { Play, Pause, Square, Timer, ChevronDown, Check } from 'lucide-react';
...
export const WorkTimer: React.FC = () => {
  ...
  const accumulatedRef = useRef<number>(0); // accumulated seconds from previous running segments
  ...
  const handleStop = useCallback(() => {
    if (elapsed < 10) { // less than 10 seconds — ignore
      setState('idle');
      setElapsed(0);
  ...
  }, [elapsed, selectedProjectId, selectedProject, selectedTaskId, selectedTask, addTimerSession]);

  const progressPct = Math.min(100, (elapsed % 3600) / 3600 * 100);
      ...
      <div className="px-5 pt-5 pb-4 border-b border-card-border flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={clsx('w-8 h-8 rounded-xl flex items-center justify-center',
            state === 'running' ? 'bg-accent-green/20' : state === 'paused' ? 'bg-amber-500/20' : 'bg-primary-500/20'
          )}>
            <Timer className={clsx('w-4 h-4',
              state === 'running' ? 'text-accent-green' : state === 'paused' ? 'text-amber-400' : 'text-primary-400'
            )} />
          </div>
          <div>
            <h3 className="font-display font-bold text-white text-sm">Chronomètre</h3>
            <p className="text-slate-500 text-xs">Suivre le temps en direct</p>
          </div>
        </div>
        {justSaved && (
      ...
      <div className="p-5 space-y-4">
            ...
            <div className="relative w-32 h-32">
              <svg className="w-32 h-32 -rotate-90" viewBox="0 0 128 128">
                ...
                <circle
                  ...
                  stroke={state === 'running' ? '#10b981' : state === 'paused' ? '#f59e0b' : '#7c3aed'}
                  strokeWidth="8"
                  strokeLinecap="round"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className={clsx('font-mono font-bold text-2xl',
                  state === 'running' ? 'text-accent-green' : state === 'paused' ? 'text-amber-400' : 'text-white'
                )}>
                  {fmt(elapsed)}
                </span>
        ...
        <div className="flex items-center justify-center gap-3">
          {state === 'idle' && (
            <button
              onClick={handleStart}
              className="flex items-center gap-2 px-6 py-2.5 bg-accent-green/20 border border-accent-green/40 text-accent-green font-semibold text-sm rounded-xl hover:bg-accent-green/30 transition-all"
            >
